...
### Optional $this->tn_size will let you change the width of the thumbnail produced
  function dcm_to_tn() {
    $filesize = 0;
    //$this->tn_file = tempnam(sys_get_temp_dir(), '/') . explode(".", $this->file)[0] . '_tn.jpg';
...
### to get exact/valid file name
function get_valid_file_name($file) {
  ...
  $filenamestr = preg_replace("/[^ \w]+/", "", $filenamestr);
  return $filenamestr;
}
